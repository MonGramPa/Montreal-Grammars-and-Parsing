...
3. How to choose rule probabilities for a grammar $$G$$ which maximizes the probability of an unparsed corpus $$W$$.
...
$$\displaystyle\sum_{B,C \in \mathbf{N}}P(A \to B C) + \sum_{w \in \mathbf{W}}P(A \to w) = 1$$.

## Inside and outside probabilities

Given a sentence $$W = w_{1m}$$, define:

**Outside probabilities.** $$\alpha_A(p, q) = P(S \Longrightarrow w_{1(p-1)} A w_{(q+1)m} \mid G)$$

The probability that substring $$w_{pq}$$ is a constituent with root category $$A$$.

**Inside probabilities.** $$\beta_A(p, q) = P(A \Longrightarrow w_{pq} \mid G)$$
...
Without a parsed corpus, this cannot be calculated directly.
...
2. If $$A$$ expands to a terminal $$w$$ then
$$\displaystyle P(A \to w \mid S \Longrightarrow w_{1m}, G) =
\sum_{h=1}^{m} \frac{\alpha_A(h,h)P(A \to w_h, w_h = w) }{\pi}=
\sum_{h=1}^{m} \frac{\alpha_A(h,h)P(w_h = w)\beta_A(h,h) }{\pi}$$
...
2. $$\hat{P}(A \to w) = \frac{P(A \to w \mid S \Longrightarrow w_{1m}, G)}{E(A \text{ is used in } S \Longrightarrow w_{1m})}$$
...
2. $$\hat{P}(A \to w) = \frac{\sum_{i=1}^\omega P(A \to w \mid S \Longrightarrow w_{1m}, G)}{\sum_{i=1}^\omega E(A \text{ is used in } S \Longrightarrow w_{1m})}$$
...
2. $$\hat{P}(A \to w) = \frac{\sum_{i=1}^\omega \sum_{h=1}^{m} \alpha_A(h,h)P(w_h = w)\beta_A(h,h) }{\sum_{i=1}^\omega \sum_{p=1}^m\sum_{q=p}^m \alpha_A(p,q)\beta_A(p,q)}$$
